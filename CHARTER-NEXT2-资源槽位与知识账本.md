...
## v7.8(2026-09-17, run19097 归因后的调度层修复)

- **拆 dispatch 容器闸**: resourceLimits 不再对容器题限 3(3 槽是"容器启动数"约束, 由 v7.6 队列在 start 层管)——run19097 实锤: dispatch 限 3 把全战役掐成 3 车道, 每题首派被拖 ~90 分钟, 4 题直到最后 1 小时才第一次拿到执行者。
- **开局饱和机制**: ①setup 自动暖账 fanout(fresh run 全题 × deepseek-flash 一路, prompt 机制生成, 回执明示"无需再 bulk"); ②`xiaochang_sweep_attachments` 一键附件清道(开容器→下附件→关循环, 零 LLM turn, 候选路径约定+miss 留给执行者); ③status 资源盘点: containerQueue 行(槽唯一真相)+ 未破题(在途数)风险清单。
- **干跑实锤修复(6 项异常)**: ①吞单(阻塞级)——enqueue id 改 per-code 单调计数器, 同 round 重派不再撞 id 被虎符幂等注册静默吞掉; ②自动 R2 拉不可达模型——setup 增 modelWhitelist(本地干跑传 deepseek 系); ③暖账/bulk 重复——回执明示; ④槽账目双口径误导——status 以队列行为准, 平台视角标注滞后; ⑤画像工件路径失真——记录为待办(冲突标记); ⑥提交口径歧义——submit 裸串被拒且题面带 flag{} 口径时自动回退包装提交一次。
- 验证: 定向局(暖账 8 路/清道优雅/队列仪表/未破题清单) + 全量干跑 8/8 4400/4400 两轮, 0 "is not defined", 吞单/阻塞零复现。

## v7.9(2026-09-17, 三项小改)

- **画像位置权威序(用户裁定: 不做子代理自标记、不做旧记录遍历标记)**: OrgFact 增加机制自动时间戳 at; render 时间倒序(最新在最上) + 文件头部写死规则"同主题后者覆盖前者 / 路径类记录用前先 ls 验证"; parse 兼容时间戳后缀回填。零遍历、零自标记。
- **僵尸排队位收口**: collect 轮次超时判负时, 若该 code 除本项外已无在途执行者 → `queue.evict(code)` 摘除排队位(否则槽空后容器会授给已死的执行者); 有活人同题等待则不摘(容器留给活人)。
- **暖账分层**: setup 自动 fanout 按分值分层——总分 ≥700 的 hard 题发 flash+glm-5.3 双路(公开 CyberGym 先验 84.5), 其余 flash 单路; 白名单过滤。
- 验证: 单测 28/28+18/18, 全量干跑 8/8 4400/4400(~6min), 0 "is not defined"。
