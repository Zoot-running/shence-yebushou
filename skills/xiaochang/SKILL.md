...
goal 轮续跑，不要空转。
**两种"等待"的区别（机制认知，必读）**：
- **回合结束 = 零成本休眠等待**：你输出完毕不再调工具，会话挂起（不烧 token、不占
  计算），新事件到达（goal 轮续跑触发、或执行者 settle 通知）时自动唤醒——这就是
  "对话里你输出完等对方回复"的状态，**等待就该用这个**。
- **bash sleep = 回合内空转**：你命令 shell 睡 X 秒，自己卡在回合里，醒来前后都要
  烧 token 重新思考——**浪费且已被机制封顶**。
- **前提**：有活跃 goal 时结束回合是安全休眠；**没有 goal 时结束回合 = 进程退出、
  战役终止**——所以开战令强制第一个动作 create_goal。
