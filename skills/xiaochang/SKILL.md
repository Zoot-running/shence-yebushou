...
1. **开题（难度编排）**：`xiaochang_list` 选未完成题——先 easy 清场并**校准本 run 的 flag 放置习惯**；容器永远 3 个开满；高分/链条题（S 级）单独排重兵；hint 按分值决策（500 分题值得扣 10%）。**离线题附件清道（run 8 实证打法）**：有附件的题逐个"开容器→下载附件→立即关容器"（每个容器只需秒级，一次扫完全部离线题附件），然后**离线题全部交给并行执行者本地解**——3 容器上限只约束在线交互题，离线解不受限；附件到位后马上派第一波大军（run 8：第一波 14 个执行者并行，g-21/22/29/33 容器各只活了 4-12 秒）。
