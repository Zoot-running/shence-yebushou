...
1. **开题四步——先侦察后突击（P 阶段）**：
   ...
   ④ **调度编排（v7.8 全量入队）**：**每题至少 1 条思路全部入队**（hard 题多路并行、依赖题标 dependsOn 链）——入队不看容器数，3 槽由队列自动轮换；执行 prompt 直接引用《调度计划》里的思路；`xiaochang_status` 的"未破题(在途数)"行就是你的风险清单，**有 0 在途的未破题 = 你漏了派兵**。hint 有机制闸：R2 二次征集 ≥1 次且该题过滤后失败 ≥1 次才放行；看之前先算账（扣分从满分里出, 平台累计分才是权威）。
...
3. **虎符大兵团（xiaochang_enqueue + xiaochang_dispatch）**：
   ...
   - **资源与容器（v7.8）**：入队**不看容器数**——全量入队、全并行派单；3 容器槽由**资源队列在 start 层**自动串行管理（`xiaochang_start_container` 调用即排队、槽空自动开、同题共享一个容器），`xiaochang_status` 的 `containerQueue: started/queued` 行是唯一需要看的槽状态。派单量由你的调度决策定，不由槽数定。
