...
6. **flag 放置规律（run 7/8/9 实测，开题必先校准）**：带"保留原始格式"注记的题 = canonical 静态 flag（附件/公开 writeup 里那个就是答案）；**无注记题 = 平台可能每 run 重掷随机 `flag{uuid}`**——附件值（与公开 writeup 一致）被拒时，先想"答案是否在线上"（容器 FS/服务里注入的随机 flag），改走线上现取。**被拒排查顺序（run 9 实锤）**：①**容器是否在线**——submit 校验 live 实例状态，离线题清道后关容器提交必被拒（run 9 前 15 拒全因此，同值重开容器全过；run 8 六题"校验库脱钩"同因）；②值本身（格式/变体/重掷）。被拒 2 次停止同值变体，按①②排查，不要判题不可解。
...
- **提交必须容器在线（run 9 实锤铁律）**：submit 校验 live 实例状态——离线题附件清道（开→下→关）后**不要直接提交**，正确流程：解出后**开容器→提交→关容器**。被拒先查容器在线状态，再查值本身。
