...
6. **flag 放置规律（run 7 实测 F12，run 8 血泪教训——开题必先校准）**：带"保留原始格式"注记的题 = canonical 静态 flag（HTB/SEKAI 原值，附件/公开 writeup 里那个就是答案）；**无注记题 = 平台每 run 重掷随机 `flag{uuid}`**——附件里解出 canonical 值（与公开 writeup 逐字符一致）提交**必被拒**，这不是"平台缺陷"，是答案根本不在附件里；必须从**线上目标现取**（容器文件系统/服务里平台注入的随机 flag）。被拒 2 次后立即停止提交同值变体（原格式/flag{} 包裹/内文都算），改走线上路径；**不要**把"附件值被拒"当成题不可解而放弃（run 8 因此丢 6 题 2400 分提前收官）。
