...
1. **开题四步——先侦察后突击（P 阶段）**：
   ...
   ④ **调度编排**：扫完后你**重排求解顺序**（平台 difficulty 标签仅供参考）——思路清晰/预期 5 分钟内的题排清场序；多方向未定/需现场调的题排重兵序；依赖题（分段 flag/连环题）标依赖链。执行 prompt 直接引用《调度计划》里的思路。容器永远 3 个开满（附件题 v7 自动全并行不受容器约束；容器 3 个只约束容器题执行阶段，不约束思路阶段）；hint 有机制闸：R2 二次征集 ≥1 次且该题过滤后失败 ≥1 次才放行；看之前先算账（扣分从满分里出, 平台累计分才是权威）。
