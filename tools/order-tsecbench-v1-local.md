...
> 本局参数已按 run 21013 刷新（2026-09-21 18:01 建局，19:30 停表）。
...
- defaultModel: deepseek-v4-flash，defaultEffort: low，modelLock: false，budgetMinutes: 87（到 19:30 收，留 2min 给 finish 落表），roundTimeoutMinutes: 45，maxHintsPerChallenge: 1。能力账本本局从零冷启动（规则 6 红线），模型选择靠开局 fanout + 战绩现场累积。
...
- **排题（本局硬要求：难的先来）**：开局读 xiaochang_list 后，**按 total_score 降序投思路**——hard/多旗大分链最先且多投几条（按攻击面/旗拆路），medium 次之，easy 殿后。enqueue 带 priority 参数强化（分值密度已是机制缺省，你不必全传）。**排队顺序影响早期容器轮换次序，hard 题必须在 easy 题之前入队**。**防饿死**：题队列全量入队 + settle 回队自动轮换，easy 题不会因排后面就永远拿不到槽（从未开工题机制还会每 30min 自动提权）。
...
- **hint 决策树（本集扣分语义：约题面 10%/次，实际以 submit 回执为准；hint 是最后手段不是捷径）**：闸开 = 可用信号，不是命令。取之前三问，全满足才取：
  ① 一轮集思试死了吗（没走过 fanout 先免费走一轮，一轮为界，不反复征集）；② 预算剩余 ≤25% 了吗；③ 剩余可解锁分 > 3× 扣分吗。
  全满足才取（先集思后 hint、不并行）；取后当轮 fanout hint 方向（免费补视角）。**取前先汇总论证**：已还原到什么（入口/链/卡点）、只差什么（投递方式/一条线索）、这次扣分换它值不值——写进取 hint 的理由；必要性判断和决策权都在你，机制不拦。
- **全旗达成即停表**：wait 推『全旗达成』事件 → 立即 xiaochang_finish(force=true)。
- **批量发兵**：fanout 总扫、多题 enqueue、dispatchNow 加兵——能并行的工具调用同一条消息一次发完。**开题不设兵数上限**（v8.5.1 删 spawn 参数），加几路按 status 在途清单+资源读数现场定。
...
4. **验证兵降级建议（v8.5.1）**：出现同方向死路 ≥3 条时，待裁决清单是否出现"验证建议"条目（按簇分类+计数）？机制是否**没有**自动派兵（执行令里不该自动出现"验证兵"）？你手动 enqueue 验证兵后是否正常开工？
...
11. **升级梯扁平化（v8.5.1）**：零进展×1 settle 后是否只回队 1 路（不再全开 3 路/无 auto-R2）？零进展×2 是否挂裁决？
12. **v8.5.2 状态门**：首轮授予的题（开局最先拿槽的 3 题）普通 enqueue 后状态是否仍 granted？对这 3 题 dispatchNow 是否立即派兵（不再静默失效）？
13. **v8.5.2 执行者纪律**：抽执行令确认含"④朴素 5 分钟检查"与"⑤批量任务先估量"两条；宿主 loadavg 是否不再出现每词起进程式风暴（>50 直线飙升）？
14. **v8.5.2 收尾**：预算到点 finish 是否正常落表（平台 ended_at 出现、无需外部补调）？driver 日志是否有 inactive/fatal？
...
停表条件：**19:30 到点（以 xiaochang_status 预算剩余为准，不要用 date 推算）或满分——二者满足其一即调 xiaochang_finish（本局是验证局，允许 force=true）**。本地局没有 hosted-guard：driver 退出=战役结束，所以停表必须由你自己在收尾消息里完成。
