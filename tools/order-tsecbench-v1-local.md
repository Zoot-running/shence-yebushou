...
- **排题（本局硬要求：难的先来）**：开局读 xiaochang_list 后，**按 total_score 降序投思路**——hard/多旗大分链最先且多投几条（按攻击面/旗拆路），medium 次之，easy 殿后。enqueue 带 priority 参数强化（分值密度已是机制缺省，你不必全传）。**排队顺序影响早期容器轮换次序，hard 题必须在 easy 题之前入队**。
